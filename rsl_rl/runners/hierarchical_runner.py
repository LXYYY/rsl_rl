...
class HierarchicalRunner(BaseRunner):
    ...
    def init_networks(self):
        ...
        high_actor_critic.std= high_actor_critic.std.to(self.device)
        ...
        mid_actor_critic.std = mid_actor_critic.std.to(self.device)
        ...
        low_actor_critic.std = low_actor_critic.std.to(self.device)
    ...
    def learn(self, num_learning_iterations, init_at_random_ep_len=False):
        ...
        ep_infos = []
        rewbuffer = deque(maxlen=int(self.num_steps_per_env))
        lenbuffer = deque(maxlen=int(self.num_steps_per_env))
        mid_lenbuffer = deque(maxlen=int(self.num_steps_per_env))
        mid_rewbuffer = deque(maxlen=int(self.num_steps_per_env))
        low_rewbuffer = deque(maxlen=int(self.num_steps_per_env))
        low_lenbuffer = deque(maxlen=int(self.num_steps_per_env))
        ...
        stds_r=torch.ones(3, dtype=torch.float32, device=self.device)/3
            ...
            for train_step in range(self.num_steps_per_env):
                with torch.inference_mode():
                    ...
                    if hi == 0:
                        ...
                        self.high_alg.actor_critic.std[:] = stds_r[0]
                    ...
                    if mi == 0:
                        ...
                        self.mid_alg.actor_critic.std[:] = stds_r[1]
                    ...
                    self.low_alg.actor_critic.std[:] = stds_r[2]
                    ...
                    obs, privileged_obs, high_rewards, dones, infos = self.env.step(low_actions, low_timeout,
                                                                                    mid_timeout, allow_reset=high_update)
                ...
                if high_return == self.high_batch_n:
                    ...
                    if self.meta_learner is not None:
                        ...
                        stds_r, meta_loss = self.meta_learner.update(all_actions, (high_rewards+high_penalty).unsqueeze(1), stds_r)
                    high_return = 0
